...
function shareOn(type) {
    ...
    const text = 'כרטיס הביקור הדיגטלי של דר מתאני מוחמד:';
...
function updateWorkingHoursStatus() {
    ...
    if (workingHours[dayOfWeek].includes('Closed')) {
        if (currentLanguage === 'hebrew') {
            ...
            statusOpenElement.textContent = 'закрыто';
            statusUntilElement.textContent = ' Откроется в 08:00';
        }
    if(dayOfWeek==5 || dayOfWeek==6){
        ...
        statusUntilElement.textContent += " Sun "
     }
     else if (currentLanguage === 'russian') {
        statusUntilElement.textContent += " воскресенье "
     }
    }
        ...
        if (currentLanguage === 'hebrew') {
            ...
            statusUntilElement.textContent = ' until 17:00 ';
        }
        else if (currentLanguage === 'russian') {
            statusOpenElement.textContent = 'открыть';
            statusUntilElement.textContent = ' до 17:00 ';
        }
...
$(document).ready(function() {
    ...
    $("#open-video-dialog-1").click(function (e) {
      e.preventDefault();
      openVideoDialog("k-bbDwc3IcI", "1"); // YouTube video ID for the first video
    });
